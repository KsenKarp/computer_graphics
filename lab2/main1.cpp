...
uchar get_new_color(uchar pixel, int n);
...
uchar get_new_color(uchar pixel, int n) {
    int color_count = 1 << n;
    int color_step = round(255.0 / (color_count - 1));
    int new_color = color_step * round(double(pixel) / color_step);
    uchar quantized_pixel = static_cast<uchar>(new_color);
    return new_color;
}
...
void FloydSteinberg(Mat& image, int n)
{
    ...
    if (image.channels() != 1) 
    { //color img
        out_img = Mat::zeros(image.size(), CV_8UC3);
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                for (int i = 0; i < 3; i++) {
                    quantized_pixel[i] = get_new_color(pixel[i], n);
                }
    ...
    { //grayscale img
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                uchar quantized_pixel = get_new_color(pixel, n); //квантуем
